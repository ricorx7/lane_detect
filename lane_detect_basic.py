...
if __name__ == '__main__':
    ...
    resize_h, resize_w = 160, 120

    verbose = False
    if verbose:
        plt.ion()
        figManager = plt.get_current_fig_manager()
        #igManager.window.showMaximized()

    # test on images
    #test_images_dir = join('data', 'test_images')
    test_images_dir = '/Users/rico/ricar_donkey/rpi/data/coleman/tub_22_17-10-01_lane_detection/'
    ...
    for test_img in test_images:
        ...
        in_image = cv2.cvtColor(cv2.imread(test_img, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)      # Convert go BGR
        out_image = color_frame_pipeline([in_image], solid_lines=True)                          # Image Pipeline to convert
        cv2.imwrite(out_path, cv2.cvtColor(out_image, cv2.COLOR_RGB2BGR))                       # Convert back to RGB
        if verbose:
